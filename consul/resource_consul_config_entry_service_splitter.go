...
func (s *serviceSplitter) Write(ce consulapi.ConfigEntry, d *schema.ResourceData, sw *stateWriter) error {
	...
	for i, s := range sp.Splits {
		...
		addHeaders := func(modifier *consulapi.HTTPHeaderModifiers, path string) {
			...
			shouldSet := func() bool {
				splits := d.Get("splits").([]interface{})
				if len(splits) <= i {
					return true
				}
				if _, found := splits[i].(map[string]interface{})[path]; !found {
					return false
				}

				return len(splits[i].(map[string]interface{})[path].([]interface{})) != 0
			}()

			if !shouldSet && len(modifier.Add)+len(modifier.Set)+len(modifier.Remove) == 0 {
				return
			}
